...
EVAL_SPLIT="${EVAL_SPLIT:-train_telephone}"
EVAL_CONTENT_SCENES="${EVAL_CONTENT_SCENES:-[\"apartment_0\",\"frl_apartment_0\"]}"
EVAL_SHUFFLE="${EVAL_SHUFFLE:-True}"
TEST_EPISODE_COUNT="${TEST_EPISODE_COUNT:-6}"
...
EVAL_ONLY_OPTS=(
  ...
  TASK_CONFIG.DATASET.SPLIT "${EVAL_SPLIT}"
  TEST_EPISODE_COUNT "${TEST_EPISODE_COUNT}"
  TASK_CONFIG.TASK.SENSORS "['SPECTROGRAM_SENSOR','AUDIOGOAL_SENSOR']"
  TASK_CONFIG.DATASET.CONTENT_SCENES "${EVAL_CONTENT_SCENES}"
  TASK_CONFIG.ENVIRONMENT.ITERATOR_OPTIONS.SHUFFLE "${EVAL_SHUFFLE}"
